<!DOCTYPE html>
<html lang="en">
<!-- Community First Bank, N.A. title.php v2.0 by Nate Holcomb -->
<!-- Sept. 9th, 2020 --> 

<!--Description: sitemap.php - Sitemap - Community First Bank, N.A.-->
<head>
	<title> Site Map - Community First Bank, N.A. </title>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<!-- BOOTSTRAP LINK, GOOGLE API LINK, JQUERY/JS LINK-->
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
	<link rel="stylesheet" href="style/style.css">
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
	<!-- Warning - JQuery is included but believed to be unused. -->
	<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
	<!-- end linking. if you want to add more files, declarations go below -->
</head>

<!--MAKING THE MAIN CONTAINER-->
<div class="container-fluid no-pad">

	<!-- making the navbar -->
	<div class="header">
		<a href="index.php">
			<img id="logo" src="media/lnl.png" alt="Community 1st Bank">
		</a>
		<h3 id="padtext"><b>Community First Bank, N.A.</b></h3>
		<div class="topnav">
			<a href="safety.php">Financial Safety</a>
			<a href="loans.php">Loans and Lending</a>
			<a href="newsletter.php">Coffee Chats</a>
			<a href="about.php">About Us</a>
			<a href="index.php">Home</a>
		</div>
	</div>

	<!--begin main content -->
	<div class = "container">
		<div class="jumbotron text-center">
			<h1> Site Map </h1>
			<br>
			<p> Find what you're looking for!</p>
		</div>
		<br>
		<div class = "row">
			<div class = "col-sm-6 text-center">
				<p> Placeholder text. Site links go here!</p>
				<a href = "index.php"> Home! </a>
				<a href = "about.php"> About! </a>
				<a href = "sitemap.php"> Sitemap! </a>
			</div>
		</div>
			
		<!-- Add sites as they are finished here. -->
		
	</div>




	<br>
	<br>
	<br>
	
	<footer class="footer">
		<div class = "container">
			<span class = "text-muted"> Copyright 2020 Community First Bank, N.A.</span>
		</div>
	</footer>

</div>

</html>
